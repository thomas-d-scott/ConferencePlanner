...
/**
 * Component to get talks based on tag.
 */
export default class TalkTagSearch extends Component {
    /**
    ...
    constructor(props) {
      ...
      /**
       * Assigning the value of tag input to the state, and fetching the data and storing in talks in the state.
       */
    handleChange(e) {
        ...
        this.setState({tag: e.target.value},  () => {
            ...
            if (this.state.tag === '') {
                fetch(`/talks/tag/0`) // This will make sure the page gets zero returns in talks, thus the page will show no result. It shows all results otherwise if you undo the input as it would just go to the endpoint to get all talks.
                .then(res => res.json())
                .then(talks => this.setState({talks}, () => console.log('Talks fetched...', talks)))
            } else {
                fetch(`/talks/tag/${this.state.tag}`)
                .then(res => res.json())
                .then(talks => this.setState({talks}, () => console.log('Talks fetched...', talks)))
            }
        });
    ...
    render() {
        return (
            <div className="container mb-3">
                <h2 className="text-center">Search by Tag</h2>
                <hr className="my-4"/>

                <div className="input-group">

                    <input type="text" name="searchTag" id="searchTag" className="form-control" placeholder='Enter Tag Here...' onChange={this.handleChange}/>
                ...
                <div className="row">
                    {/* For each talk in Talks array, calling talk component */}

...
/**
 * Component to find talks based on speaker
 */
export default class TalkSpeakerSearch extends Component {
    /**
    ...
    constructor(props) {
      ...
      /**
       * Sets speaker state to value in the input box then fetches data and stored results in talks array in state 
       */
    handleChange(e) {
        ...
        this.setState({speaker: e.target.value},  () => {
            ...
            if (this.state.speaker === '') {
                fetch(`/talks/speaker/0`) // This will make sure the page gets zero returns in talks, thus the page will show no result. It shows all results otherwise if you undo the input as it would just go to the endpoint to get all talks.
                .then(res => res.json())
                .then(talks => this.setState({talks}, () => console.log('Talks fetched...', talks)))
            } else {
                fetch(`/talks/speaker/${this.state.speaker}`)
                .then(res => res.json())
                .then(talks => this.setState({talks}, () => console.log('Talks fetched...', talks)))
            }
        });
    ...
    render() {
        return (
            <div className="container mb-3">
                <h2 className="text-center">Search by Speaker</h2>
                <hr className="my-4"/>

                <div className="input-group">

                    <input type="text" name="searchSpeaker" id="searchSpeaker" className="form-control" placeholder='Enter Speaker Here...' onChange={this.handleChange}/>
                ...
                <div className="row">
                    {/* Loads Talk component for each talk in the talks array */}

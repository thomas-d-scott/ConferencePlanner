...
/**
 * Component to get the talks based on description
 */
export default class TalkDescriptionSearch extends Component {
    /**
    ...
    constructor(props) {
      ...
      /**
       * Set the description in state to the value the user entered, then fetches talks based on this and stores in talks state
       */
    handleChange(e) {
        ...
        this.setState({description: e.target.value},  () => {
            ...
            if (this.state.description === '') {
                fetch(`/talks/description/0`) // This will make sure the page gets zero returns in talks, thus the page will show no result. It shows all results otherwise if you undo the input as it would just go to the endpoint to get all talks.
                .then(res => res.json())
                .then(talks => this.setState({talks}, () => console.log('Talks fetched...', talks)))
            } else {
                fetch(`/talks/description/${this.state.description}`)
                .then(res => res.json())
                .then(talks => this.setState({talks}, () => console.log('Talks fetched...', talks)))
            }
        });
    ...
    render() {
        return (
            <div className="container mb-3">
                <h2 className="text-center">Search by Description</h2>
                <hr className="my-4"/>

                <div className="input-group">

                    <input type="text" name="searchDescription" id="searchDescription" className="form-control" placeholder='Enter Description Here...' onChange={this.handleChange}/>
                ...
                <div className="row">
                    ...
                    {/* For each talk in talks array, calls the Talk Component */}
